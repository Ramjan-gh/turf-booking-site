...
export function CheckBooking() {
  ...
  const discountAmount = data.booking.discount_amount || 0;
  const pointRedeemAmount = data.booking.point_redeem_amount || 0;
  const finalAmount = data.booking.final_amount || 0;
const paidAmount = Math.min(data.booking.paid_amount || 0, finalAmount);
const dueAmount = Math.max(0, finalAmount - paidAmount);
  ...
  const mappedBooking: Booking = {
    ...
    paymentAmount: data.booking.payment_status === "pertially_paid" ? "confirmation" : "full",
...
<div className="mt-12 pt-8 border-t border-slate-100">
  <p className="text-sm text-slate-400 font-medium mb-4">Payment Summary</p>
  <div className="space-y-2 mb-4">
    <div className="flex justify-between text-sm text-slate-500">
      <span>Subtotal ({booking.slots.length} slot{booking.slots.length !== 1 ? "s" : ""}):</span>
      <span className="font-mono font-semibold text-slate-700">৳{booking.totalPrice.toLocaleString()}</span>
    </div>
      ...
      <div className="flex justify-between text-sm text-green-600">
        <span className="flex items-center gap-1.5">
          ...
          {booking.discountCode && (
            <span className="font-mono bg-green-50 text-green-700 px-1.5 py-0.5 rounded text-[10px] font-bold border border-green-100">
              {booking.discountCode}
            </span>
          )}:
        </span>
        <span className="font-mono font-semibold">-৳{(booking.discountAmount ?? 0).toLocaleString()}</span>
      </div>
    )}

    {(booking.pointRedeemAmount ?? 0) > 0 && (
      <div className="flex justify-between text-sm text-emerald-600">
        <span>✨ Loyalty points redeemed:</span>
        <span className="font-mono font-semibold">-৳{(booking.pointRedeemAmount ?? 0).toLocaleString()}</span>
      </div>
    )}

    <div className="flex justify-between text-sm font-semibold text-slate-700 border-t border-dashed border-slate-100 pt-2">
      <span>Net Required Amount:</span>
      <span className="font-mono">৳{(booking.finalAmount ?? booking.totalPrice).toLocaleString()}</span>
    </div>
  </div>

  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div className="p-4 bg-green-50 rounded-2xl border border-green-100 text-center">
      <p className="text-[10px] uppercase font-bold text-green-600 tracking-widest mb-2">Paid</p>
      <p className="text-2xl font-black text-green-700">৳{booking.paidAmount.toLocaleString()}</p>
    </div>
    <div className={`p-4 rounded-2xl border text-center ${booking.dueAmount > 0 ? "bg-amber-50 border-amber-100" : "bg-slate-50 border-slate-100"}`}>
      <p className={`text-[10px] uppercase font-bold tracking-widest mb-2 ${booking.dueAmount > 0 ? "text-amber-600" : "text-slate-400"}`}>
        Due at Venue
      </p>
      <p className={`text-2xl font-black ${booking.dueAmount > 0 ? "text-amber-700" : "text-slate-400"}`}>
